...
, mpHost(pHost)
{
    ...
    lineEdit_logFileFolder->setToolTip(tr("Location which will be used to store log files - matching logs will be appended to."));
    lineEdit_logFileFolder->setAccessibleName(tr("Log file folder"));
    lineEdit_logFileFolder->setAccessibleDescription(tr("Location which will be used to store log files - matching logs will be appended to."));
    pushButton_whereToLog->setToolTip(tr("Select a directory where logs will be saved."));
    pushButton_whereToLog->setAccessibleName(tr("Browse for log folder"));
    pushButton_whereToLog->setAccessibleDescription(tr("Select a directory where logs will be saved."));
    pushButton_resetLogDir->setToolTip(tr("Reset the directory so that logs are saved to the profile's log directory."));
    pushButton_resetLogDir->setAccessibleName(tr("Reset log directory"));
    pushButton_resetLogDir->setAccessibleDescription(tr("Reset the directory so that logs are saved to the profile's log directory."));
    ...
    comboBox_logFileNameFormat->setAccessibleName(tr("Log file name format"));
    comboBox_logFileNameFormat->setAccessibleDescription(tr("This option sets the format of the log name. If Named file is selected, you can set a custom file name."));
    lineEdit_logFileName->setToolTip(tr("Set a custom name for your log. (New logs are appended if a log file of the same name "
                                        "already exists)."));
    lineEdit_logFileName->setAccessibleName(tr("Log file name"));
    lineEdit_logFileName->setAccessibleDescription(tr("Set a custom name for your log. New logs are appended if a log file of the same name already exists."));
    ...
    */
    ...
    if (mudlet::self()->developmentVersion && !qEnvironmentVariableIsSet("DEV_UPDATER")) {
        ...
        checkbox_noAutomaticUpdates->setAccessibleName(tr("Disable automatic updates"));
        checkbox_noAutomaticUpdates->setAccessibleDescription(tr("Automatic updates are disabled in development builds to prevent an update from overwriting your Mudlet."));
    } else {
    ...
    pushButton_showGlyphUsage->setToolTip(tr("This will bring up a display showing all the symbols used in the current "
                                             "map and whether they can be drawn using just the specified font, any other "
                                             "font, or not at all.  It also shows the sequence of Unicode code-points "
    ...
    pushButton_showGlyphUsage->setAccessibleName(tr("Show glyph usage"));
    pushButton_showGlyphUsage->setAccessibleDescription(tr("Display all symbols used in the current map and which fonts can draw them, with examples of where they are used."));
    fontComboBox_mapSymbols->setToolTip(tr("Select the only or the primary font used (depending on 'Only use symbols "
                                               "(glyphs) from chosen font' setting) to produce the 2D mapper room symbols."));
    fontComboBox_mapSymbols->setAccessibleName(tr("Map symbol font"));
    fontComboBox_mapSymbols->setAccessibleDescription(tr("Select the primary font used to produce the 2D mapper room symbols."));
    checkBox_isOnlyMapSymbolFontToBeUsed->setToolTip(tr("Using a single font is likely to produce a more consistent style but may "
                                                         "cause the font replacement character '�' to show if the font "
    ...
    checkBox_isOnlyMapSymbolFontToBeUsed->setAccessibleName(tr("Only use symbols from chosen font"));
    checkBox_isOnlyMapSymbolFontToBeUsed->setAccessibleDescription(tr("Using a single font produces consistent style but may show replacement character if the font lacks a symbol. Uncheck to allow alternatives from other fonts."));
    ...
    if (!entries.isEmpty()) {
        ...
        for (int i = 0, total = entries.size(); i < total; ++i) {
            ...
            if (mudlet::self()->mDictionaryLanguageCodeMap.contains(key)) {
                ...
                item->setToolTip(tr("From the dictionary file <tt>%1.dic</tt> (and its companion affix <tt>.aff</tt> file).").arg(dir.absoluteFilePath(entries.at(i))));
            } else {
...
void dlgProfilePreferences::generateMapGlyphDisplay()
{
    ...
    while (itUsedSymbol.hasNext()) {
        ...
        pSymbolInFont->setToolTip(tr("The room symbol will appear like this if only symbols (glyphs) from the specific font are used."));
        pSymbolInFont->setFont(selectedFont);

        auto * pSymbolAnyFont = new QTableWidgetItem();
        pSymbolAnyFont->setTextAlignment(Qt::AlignCenter);
        pSymbolAnyFont->setToolTip(tr("The room symbol will appear like this if symbols (glyphs) from any font can be used."));
        ...
        pUsageCount->setToolTip(tr("How many rooms in the whole map have this symbol."));
        ...
        pRoomNumbers->setToolTip(tr("The rooms with this symbol, up to a maximum of thirty-two, if there are more "
        ...
        if (isSingleFontUsable) {
            ...
            pDummyButton->setToolTip(tr("The symbol can be made entirely from glyphs in the specified font."));
        } else {
            ...
            if (isAllFontUsable) {
                ...
                pDummyButton->setToolTip(tr("The symbol cannot be drawn using any of the fonts in the system, either an "
