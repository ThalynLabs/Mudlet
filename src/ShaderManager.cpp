...
QString ShaderManager::loadShaderSource(const QString& shaderName)
{
    ...
    if (source.isEmpty()) {
        qWarning() << "ShaderManager: Falling back to embedded shader source for" << shaderName;
        source = getEmbeddedShaderSource(shaderName);
    }
...
QString ShaderManager::getEmbeddedShaderSource(const QString& shaderName)
{
    if (shaderName == qsl("vertex")) {
        return R"(
#version 330 core
...
void main()
{
    ...
    } else if (shaderName == qsl("fragment")) {
        return R"(
#version 330 core
in vec4 vertexColor;
out vec4 FragColor;
